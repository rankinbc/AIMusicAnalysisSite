 ...
 * Tabs: Visuals · Coach · People · Chat · Stats · Notes.
 * One host controls the session for everyone; listeners react + chat.
 */
const { useState: useRailS, useEffect: useRailE, useRef: useRailR } = React;

// SYNC toggle shown in a console bay header — binds that module to the music
function SyncTag({ on, onChange }) {
  return (
...
function hexHue(hex) {
  ...
  const m = hex.slice(1); const r = parseInt(m.slice(0, 2), 16) / 255, g = parseInt(m.slice(2, 4), 16) / 255, b = parseInt(m.slice(4, 6), 16) / 255;
  const mx = Math.max(r, g, b), mn = Math.min(r, g, b), d = mx - mn; let h = 0;
  if (d) { if (mx === r) h = ((g - b) / d) % 6; else if (mx === g) h = (b - r) / d + 2; else h = (r - g) / d + 4; h *= 60; if (h < 0) h += 360; }
  return Math.round(h);
}

function useLiveMeters(track, playing) {
  const [v, setV] = useRailS({ lufsS: track.loudness.integrated, peak: track.loudness.truePeak, corr: track.stereo.correlation, rms: track.loudness.rms });
  useRailE(() => {
    if (!playing) { setV({ lufsS: track.loudness.integrated, peak: track.loudness.truePeak, corr: track.stereo.correlation, rms: track.loudness.rms }); return; }
    let raf = 0, t0 = performance.now() / 1000;
    const tick = () => {
...
function VisualMeters({ track, playing, open, setOpen }) {
  const lm = useLiveMeters(track, playing);
  const corrPct = (lm.corr + 1) / 2;
  if (!open) {
    return (
      <button onClick={() => setOpen(true)} className="mono" style={{ position: 'absolute', top: 12, right: 12, zIndex: 7, fontSize: 9.5, padding: '6px 10px', borderRadius: 8, background: 'rgba(7,10,18,0.72)', border: '1px solid var(--border)', backdropFilter: 'blur(10px)', color: 'var(--text-2)', display: 'inline-flex', alignItems: 'center', gap: 7 }}>
  ...
  return (
    <div style={{ position: 'absolute', top: 12, right: 12, zIndex: 7, width: 168, padding: '10px 12px', borderRadius: 10, background: 'rgba(7,10,18,0.74)', border: '1px solid var(--border)', backdropFilter: 'blur(12px)', display: 'flex', flexDirection: 'column', gap: 9 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <button onClick={() => setOpen(false)} className="mono" style={{ fontSize: 10, color: 'var(--muted)' }}>▾</button>
      </div>
...
// ── panel section label ────────────────────────────────────────────────────
function PLabel({ children, accent = 'var(--muted)' }) {
...
// ── VISUALS tab — all visualization options (host drives for everyone) ─────
function VisualsPanel({ stages, toggleStage, director, setDirector, viz, setViz, presets = [], onSave, onRecall, onRandomize }) {
  ...
  const cycleTheme = (i) => setViz((s) => { const t = [...(s.theme || theme)]; const idx = BG_COLORS.indexOf(t[i]); t[i] = BG_COLORS[(idx + 1) % BG_COLORS.length]; return { ...s, theme: t }; });
    ...
    <div className="card rack-panel" style={{ overflow: 'visible' }}>
      <div className="card-hd">
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          {onRandomize && <button onClick={onRandomize} className="btn sm" style={{ fontSize: 9.5, padding: '5px 10px' }}>⚄ Random</button>}
          {onSave && <button onClick={onSave} className="btn sm primary" style={{ fontSize: 9.5, padding: '5px 10px' }}>+ Save</button>}
        </div>
      </div>

      <div className="card-body lc-grid">
      <div className="lc-bay">
        <div className="lc-bay-hd"><span className="lc-led" style={{ background: 'var(--cyan)', boxShadow: '0 0 7px var(--cyan)' }} /><span className="lc-label">Primary actions</span></div>
        <button onClick={toggleSyncAll} className="btn sm" style={{ width: '100%', justifyContent: 'center', color: syncAllOn ? 'var(--green)' : 'var(--cyan)', borderColor: syncAllOn ? 'rgba(52,211,153,0.4)' : 'rgba(0,229,176,0.4)', marginBottom: 9 }}>{syncAllOn ? '✓ All visuals synced to music' : '⟳ Sync all to music'}</button>
        <div className="lc-label" style={{ marginBottom: 7 }}>Color sync · theme</div>
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          {theme.map((c, i) => (<button key={i} onClick={() => cycleTheme(i)} title="click to change colour" style={{ flex: 1, height: 30, borderRadius: 8, background: c, border: '2px solid rgba(255,255,255,0.2)', boxShadow: `0 0 10px ${c}66` }} />))}
          <button onClick={applyTheme} className="btn sm primary" style={{ fontSize: 9.5, padding: '6px 11px' }}>Sync</button>
        </div>
        <div className="mono" style={{ fontSize: 8.5, color: 'var(--muted)', marginTop: 6 }}>3 colours map to spectrum · lasers · background.</div>
      </div>

      <div className="lc-bay">
        <div className="lc-bay-hd"><span className="lc-led" /><span className="lc-label">Stage select</span><span className="mono" style={{ marginLeft: 'auto', fontSize: 8, color: viz.autoReact ? 'var(--cyan)' : 'var(--muted)' }}>{viz.autoReact ? 'auto-react ●' : 'combine ↻'}</span></div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 6 }}>
          {STAGES.map((st) => { const on = stages.includes(st.id); return (
            <button key={st.id} onClick={() => toggleStage(st.id)} title={st.label} className={'lc-cell' + (on ? ' on' : '')}>
              ...
              <span className={'lc-led' + (on ? '' : ' off')} style={{ width: 5, height: 5 }} />
            </button>
          ); })}
        </div>
      </div>
      ...
      <div className="lc-bay">
        <div className="lc-bay-hd"><span className={'lc-led' + (director === 'off' ? ' off' : '')} style={{ background: director === 'off' ? undefined : 'var(--violet)', boxShadow: director === 'off' ? undefined : '0 0 7px var(--violet)' }} /><span className="lc-label">Auto program</span><span className="mono" style={{ marginLeft: 'auto', fontSize: 8, color: 'var(--muted)' }}>sets modules</span></div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 6 }}>
          {DIRECTORS.map((d) => { const on = director === d.id; return (
            <button key={d.id} onClick={() => setDirector(d.id)} title={d.blurb} className={'lc-cell' + (on ? ' on' : '')}>
              ...
              <span className={'lc-led' + (on ? '' : ' off')} style={{ width: 5, height: 5, background: on ? 'var(--violet)' : undefined, boxShadow: on ? '0 0 6px var(--violet)' : undefined }} />
            </button>
          ); })}
        </div>
        <div className="mono" style={{ fontSize: 9.5, color: 'var(--muted)', lineHeight: 1.4 }}>{dirObj.blurb}</div>
      </div>

      <div className="lc-bay">
        <div className="lc-bay-hd"><span className={'lc-led' + (viz.autoReact ? '' : ' off')} style={{ background: viz.autoReact ? 'var(--cyan)' : undefined, boxShadow: viz.autoReact ? '0 0 7px var(--cyan)' : undefined }} /><span className="lc-label">Auto-react to music</span><span style={{ marginLeft: 'auto' }}><Switch on={viz.autoReact} onChange={(v) => setViz((s) => ({ ...s, autoReact: v }))} /></span></div>
      ...
      <div className="lc-bay">
        <div className="lc-bay-hd"><span className={'lc-led' + (viz.laserOn ? '' : ' off')} /><span className="lc-label">⚡ Laser rig</span><span style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 12 }}><SyncTag on={viz.laserSync} onChange={(v) => setViz((s) => ({ ...s, laserSync: v }))} /><Switch on={viz.laserOn} onChange={(v) => setViz((s) => ({ ...s, laserOn: v }))} /></span></div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}><span className="lc-label">Effect</span><div style={{ marginLeft: 'auto' }}><SelectChip value={viz.laserEffect} options={LASER_EFFECTS} onChange={(v) => setViz((s) => ({ ...s, laserEffect: v }))} /></div></div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}><span className="lc-label">Pattern</span><div style={{ marginLeft: 'auto' }}><SelectChip value={viz.laserPattern} options={LASER_PATTERNS} onChange={(v) => setViz((s) => ({ ...s, laserPattern: v }))} /></div></div>
      ...
      <div className="lc-bay">
        <div className="lc-bay-hd"><span className="lc-led" style={{ background: accentNow, boxShadow: `0 0 7px ${accentNow}` }} /><span className="lc-label">Primary color</span><span style={{ marginLeft: 'auto' }}><SyncTag on={viz.autoColor} onChange={(v) => setViz((s) => ({ ...s, autoColor: v }))} /></span></div>
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>{PALETTE.map((c) => (<button key={c} onClick={() => setViz((s) => ({ ...s, barColor: c }))} style={{ width: 22, height: 22, borderRadius: 6, background: c, border: accentNow === c ? '2px solid #fff' : '2px solid transparent', boxShadow: `0 0 8px ${c}66` }} />))}</div>
        <HueSlider value={viz.specHue} onChange={(h) => setViz((s) => ({ ...s, specHue: h, barColor: hslToHex(h) }))} />
      </div>

      <div className="lc-bay">
        <div className="lc-bay-hd"><span className={'lc-led' + (viz.bgAuto ? '' : ' off')} style={{ background: viz.bgAuto ? 'var(--violet)' : undefined, boxShadow: viz.bgAuto ? '0 0 7px var(--violet)' : undefined }} /><span className="lc-label">Background</span><span style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 12 }}><SyncTag on={viz.bgSync} onChange={(v) => setViz((s) => ({ ...s, bgSync: v }))} /><label style={{ display: 'flex', alignItems: 'center', gap: 6 }}><span className="mono" style={{ fontSize: 8, color: 'var(--muted)' }}>CYCLE</span><Switch on={viz.bgAuto} onChange={(v) => setViz((s) => ({ ...s, bgAuto: v }))} accent="var(--violet)" /></label></span></div>
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', opacity: viz.bgAuto ? 0.4 : 1, pointerEvents: viz.bgAuto ? 'none' : 'auto' }}>{PALETTE.map((c) => (<button key={c} onClick={() => setViz((s) => ({ ...s, bg: c }))} style={{ width: 22, height: 22, borderRadius: 6, background: c, border: (viz.bg || '#00e5b0') === c ? '2px solid #fff' : '2px solid transparent' }} />))}</div>
        ...
        <div style={{ opacity: viz.bgFlash ? 1 : 0.4, pointerEvents: viz.bgFlash ? 'auto' : 'none', display: 'flex', flexDirection: 'column', gap: 8 }}>
          ...
          <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}><span className="lc-label">Color</span>{['#ffffff', '#00e5b0', '#a78bfa', '#fb923c', '#f43f5e'].map((c) => (<button key={c} onClick={() => setViz((s) => ({ ...s, bgFlashColor: c }))} style={{ width: 20, height: 20, borderRadius: 5, background: c, border: viz.bgFlashColor === c ? '2px solid #fff' : '2px solid transparent' }} />))}</div>
        </div>
      </div>

      <div className="lc-bay">
        <div className="lc-bay-hd"><span className={'lc-led' + (viz.dropFx ? '' : ' off')} style={{ background: viz.dropFx ? 'var(--orange)' : undefined, boxShadow: viz.dropFx ? '0 0 7px var(--orange)' : undefined }} /><span className="lc-label">FX · Moments</span></div>
...
function CoachPanel({ rs, announce }) {
  const [applied, setApplied] = useRailS({});
  const [msgs, setMsgs] = useRailS([]);
  const [text, setText] = useRailS('');
  const ask = (q) => { if (!q.trim()) return; const r = coachReply(q); setMsgs((m) => [...m, { who: 'you', text: q }, { who: 'coach', text: r.text, fix: r.fix, apply: r.apply }]); setText(''); };
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10, maxHeight: 380, overflow: 'auto', paddingRight: 2 }}>
        {msgs.map((m, i) => (
          <div key={i} className="fade-in" style={{ display: 'flex', gap: 8, flexDirection: m.who === 'you' ? 'row-reverse' : 'row' }}>
            {m.who === 'coach' ? <CoachBot size={24} /> : <Avatar handle="maek" hue={168} size={24} />}
            <div style={{ maxWidth: '82%' }}>
              ...
              {m.who === 'coach' && m.apply && (
                <button onClick={() => { rs.applyCoach(m.apply); setApplied((a) => ({ ...a, [i]: true })); announce && announce(`Applied — ${m.fix}`); }} className="btn sm" style={{ marginTop: 6, color: applied[i] ? 'var(--green)' : 'var(--cyan)', borderColor: applied[i] ? 'rgba(52,211,153,0.4)' : 'rgba(0,229,176,0.4)' }}>{applied[i] ? `✓ Applied — ${m.fix}` : 'Apply suggestion'}</button>
              )}
            </div>
          </div>
        ))}
      ...
      <div style={{ display: 'flex', gap: 7, alignItems: 'center', padding: '7px 10px', borderRadius: 8, border: '1px solid var(--border)' }}>
        ...
        <button onClick={() => ask(text)} className="btn sm primary" style={{ padding: '4px 12px', fontSize: 11 }}>Ask →</button>
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <button className="btn ghost sm" style={{ color: 'var(--cyan)', fontSize: 10 }}>↗ Invite</button>
      </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
          {REACTION_GROUPS.map((g) => (
            <div key={g.id} style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
              ...
              {g.emojis.map((e) => (
                <button key={e} onClick={() => onReact && onReact(e)} title={`Set status — ${g.label.toLowerCase()}`} style={{ flex: 1, padding: '5px 0', borderRadius: 6, fontSize: 14, background: myStatus === e ? `${cssVar(g.tone)}26` : 'rgba(255,255,255,0.02)', border: `1px solid ${myStatus === e ? cssVar(g.tone) : 'var(--border)'}` }}>{e}</button>
              ))}
            </div>
          ))}
        </div>
          ...
          <div key={u.handle} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '7px 9px', borderRadius: 8, background: 'rgba(255,255,255,0.02)', border: `1px solid ${u.you ? 'rgba(0,229,176,0.4)' : 'var(--border)'}` }}>
            ...
            <span style={{ marginLeft: 'auto', display: 'inline-flex', alignItems: 'center', gap: 6 }}>
              ...
              {!u.you && (rackController === u.handle
                ...
                : <button onClick={() => onGrant(u.handle, 'rack')} className="btn ghost sm" style={{ fontSize: 8, padding: '3px 6px', color: 'var(--muted)' }}>+ Rack</button>)}
              {!u.you && (visualController === u.handle
                ...
                : <button onClick={() => onGrant(u.handle, 'visuals')} className="btn ghost sm" style={{ fontSize: 8, padding: '3px 6px', color: 'var(--muted)' }}>+ Vis</button>)}
...
function ChatPanel({ feed, onReact }) {
  const [msgs, setMsgs] = useRailS([]);
  const [text, setText] = useRailS('');
  const stream = [...feed.map((f) => ({ ...f, kind: 'react' })), ...msgs, ...SEED_CHAT.map((c) => ({ ...c, kind: 'chat' }))];
  const send = () => { if (!text.trim()) return; setMsgs((m) => [{ id: Math.random().toString(36).slice(2), handle: 'maek', text: text.trim(), kind: 'chat', you: true }, ...m]); setText(''); };
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10, height: '100%' }}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 9, flex: 1, overflow: 'auto', maxHeight: 320 }}>
        ...
        {stream.map((m) => (
          <div key={m.id} className="fade-in" style={{ display: 'flex', alignItems: 'flex-start', gap: 8 }}>
            {m.emoji ? <span style={{ fontSize: 15 }}>{m.emoji}</span> : (() => { const u = ROOM_LISTENERS.find((x) => x.handle === m.handle); return <Avatar handle={m.handle} hue={u ? u.hue : 200} anon={u ? u.anon : false} size={20} />; })()}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
        {REACTION_GROUPS.map((g) => (
          <div key={g.id} style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
            ...
            {g.emojis.map((e) => (
              <button key={e} onClick={() => onReact(e)} style={{ flex: 1, padding: '6px 0', borderRadius: 7, fontSize: 15, background: `${cssVar(g.tone)}10`, border: `1px solid ${cssVar(g.tone)}2a` }}>{e}</button>
            ))}
          </div>
        ))}
      </div>
      <div style={{ display: 'flex', gap: 7, alignItems: 'center', padding: '7px 10px', borderRadius: 8, border: '1px solid var(--border)' }}>
        ...
        <button onClick={send} className="btn sm primary" style={{ padding: '4px 10px', fontSize: 11 }}>Send</button>
      </div>
    </div>
  );
}
...
function StatRow({ label, value, tone = 'var(--text)' }) {
  return (
...
function NotesPanel({ track, position, activeNote, onNoteClick }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
      ...
      {track.notes.map((n) => { const active = activeNote === n.id, c = n.pinned ? 'var(--cyan)' : 'var(--violet)'; return (
        <button key={n.id} onClick={() => onNoteClick(n)} style={{ width: '100%', textAlign: 'left', display: 'flex', gap: 10, padding: '8px 10px', borderRadius: 8, background: active ? 'rgba(0,229,176,0.05)' : 'transparent', border: active ? '1px solid rgba(0,229,176,0.32)' : '1px solid transparent' }}>
...
// ── PLAN tab — actionable fixes from the analysis (apply → coach announces) ─
function PlanPanel({ rs, announce }) {
  const [done, setDone] = useRailS({});
  return (
      ...
      {PLAN_ITEMS.map((p, i) => {
        ...
        return (
          <div key={p.id} style={{ padding: 12, borderRadius: 9, background: 'rgba(255,255,255,0.02)', border: `1px solid ${cssVar(p.color)}33`, borderLeft: `3px solid ${p.color}` }}>
            ...
            <button onClick={() => { rs.applyCoach(p.apply); setDone((s) => ({ ...s, [p.id]: true })); announce && announce(`Applied — ${p.fix}`); }} className="btn sm" style={{ marginTop: 10, width: '100%', justifyContent: 'center', color: d ? 'var(--green)' : 'var(--cyan)', borderColor: d ? 'rgba(52,211,153,0.4)' : 'rgba(0,229,176,0.4)' }}>{d ? '✓ Applied' : 'Apply fix'}</button>
          </div>
        );
      })}
    </div>
...
// ── the panel shell ────────────────────────────────────────────────────────
function RightRail({ mode, stages, toggleStage, director, setDirector, viz, setViz, rs, track, playing, position, activeNote, onNoteClick, onReact, feed, announce, myStatus, rackController, visualController, onGrant }) {
  const TABS = [['coach', 'Coach'], ['plan', 'Plan'], ['people', 'People'], ['chat', 'Chat'], ['stats', 'Stats'], ['notes', 'Notes']];
  const [tab, setTab] = useRailS(mode === 'room' ? 'chat' : 'coach');
  useRailE(() => { setTab(mode === 'room' ? 'chat' : 'coach'); }, [mode]);
  return (
    <div className="card" style={{ padding: 14, position: 'sticky', top: 16, alignSelf: 'start', display: 'flex', flexDirection: 'column', maxHeight: 'calc(100vh - 32px)' }}>
      <div style={{ display: 'flex', gap: 3, padding: 3, borderRadius: 8, border: '1px solid var(--border)', background: 'rgba(255,255,255,0.02)', marginBottom: 14, flexWrap: 'wrap' }}>
        {TABS.map(([id, label]) => (
          <button key={id} onClick={() => setTab(id)} className="mono" style={{ flex: '1 1 auto', fontSize: 9.5, fontWeight: 600, letterSpacing: '0.04em', padding: '6px 4px', borderRadius: 6, color: tab === id ? 'var(--text)' : 'var(--muted)', background: tab === id ? 'var(--card-hover)' : 'transparent', boxShadow: tab === id ? 'inset 0 0 0 1px var(--border)' : 'none', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 4 }}>
            {label}{(id === 'chat' || id === 'people') && mode === 'room' && <span style={{ width: 5, height: 5, borderRadius: '50%', background: 'var(--violet)', boxShadow: '0 0 6px var(--violet)' }} />}
          </button>
        ))}
      </div>
      <div style={{ overflow: 'auto', flex: 1, paddingRight: 2 }}>
        {tab === 'coach' && <CoachPanel rs={rs} announce={announce} />}
        {tab === 'plan' && <PlanPanel rs={rs} announce={announce} />}
        {tab === 'people' && <PeoplePanel myStatus={myStatus} onReact={onReact} rackController={rackController} visualController={visualController} onGrant={onGrant} />}
        {tab === 'chat' && <ChatPanel feed={feed} onReact={onReact} />}
        {tab === 'stats' && <StatsPanel track={track} />}
        {tab === 'notes' && <NotesPanel track={track} position={position} activeNote={activeNote} onNoteClick={onNoteClick} />}
      </div>
    </div>
  );
}

Object.assign(window, { RightRail, VisualMeters, VisualsPanel, CoachPanel, PeoplePanel, ChatPanel, StatsPanel, NotesPanel });
