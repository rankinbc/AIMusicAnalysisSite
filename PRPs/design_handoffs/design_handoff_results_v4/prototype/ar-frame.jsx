  ...
  return (
    <div className="rhead">
      ...
      <div className="rh-top">
        <CoverArt hue={scenario.track.hue} size={44} badge={scenario.track.version} />
        <div className="rh-titles">
          <div className="rh-titlerow">
            ...
            {scenario.meta && scenario.meta.genre && scenario.meta.genre !== 'other' && (
              <span className="rh-genre">{scenario.meta.genre.charAt(0).toUpperCase() + scenario.meta.genre.slice(1)}</span>
            )}
          </div>
...
function TabBar({ tab, setTab, tabs }) {
  const btn = (tb) => (
    <button key={tb.id} className={`rtab${tab === tb.id ? ' active' : ''}${tb.alert ? ' alert' : ''}${tb.hot ? ' hot' : ''}${tb.disabled ? ' disabled' : ''}`}
      aria-disabled={tb.disabled || undefined} onClick={() => { if (!tb.disabled) setTab(tb.id); }}>
      <span className="ic"><Icon name={tb.icon} size={15} /></span>
      {tb.label}
      {tb.badge != null && tb.badge > 0 && <span className="rtab-badge">{tb.badge}</span>}
      {tb.tip && <span className="rtab-tip">{tb.tip}</span>}
    </button>
  );
  const left = tabs.filter(t => !t.right), right = tabs.filter(t => t.right);
  return (
    <div className="rtabs">
      {left.map(btn)}
      {right.length > 0 && <span className="rtabs-sp" />}
      {right.map(btn)}
    </div>
  );
}
