...
function Icon({ name, size = 15 }) {
  ...
  const paths = {
    ...
    eyeoff: <><path d="M2 12s3.5-7 10-7c2.1 0 3.9.7 5.4 1.7M22 12s-3.5 7-10 7c-2.1 0-3.9-.7-5.4-1.7" /><circle cx="12" cy="12" r="3" /><path d="M4 20 20 4" /></>,
    ...
    thumbup: <path d="M7 11v9H4a1 1 0 0 1-1-1v-7a1 1 0 0 1 1-1zM7 11l4-7a2 2 0 0 1 2 2v4h5.2a2 2 0 0 1 2 2.4l-1.2 6a2 2 0 0 1-2 1.6H7" />,
    thumbdown: <path d="M17 13V4h3a1 1 0 0 1 1 1v7a1 1 0 0 1-1 1zM17 13l-4 7a2 2 0 0 1-2-2v-4H5.8a2 2 0 0 1-2-2.4l1.2-6a2 2 0 0 1 2-1.6H17" />,
    ...
    headphones: <><path d="M4 14v-1a8 8 0 0 1 16 0v1" /><rect x="3" y="14" width="4.5" height="6" rx="1.8" /><rect x="16.5" y="14" width="4.5" height="6" rx="1.8" /></>,
  };
...
// ── MiniSpectrum — 7-band evidence chart, one band flagged; optional post-fix ghost ─
function MiniSpectrum({ bands, warnIndex, fixDelta }) {
    ...
    <div>
      <div className="chart-bars">
        {bands.map((b, i) => {
          ...
          const ghost = warn && fixDelta != null;
          return (
            <div className="chart-col" key={i}>
              <div className="bw">
                {ghost && <div className="barfix" style={{ height: h(b.db + fixDelta) }} title={`after fix ≈ ${(b.db + fixDelta).toFixed(1)} dB`} />}
                <div className={`barfill${warn ? ' warn' : ''}`} style={{ height: h(b.db) }} />
              </div>
      ...
      {fixDelta != null && <div className="chart-fixleg mono"><span className="sw now" />now<span className="sw aft" />after fix ({fixDelta > 0 ? '+' : ''}{fixDelta} dB on the flagged band)</div>}
    </div>
  );
}

// ── MiniMeter — value vs target on a scale ───────────────────────────
function MiniMeter({ value, min, max, target, targetLabel, hot, after }) {
  ...
  return (
    <div>
      <div className="meter-line">
        ...
        {after != null && <div className="aftmark" style={{ left: pct(after) }} title={`predicted after fix: ${after}`}><span className="cap">after ≈ {after}</span></div>}
