...
import type { ReactionFeedItem, Track, TrackNote } from './data';
...
export function Transport({
  ...
  return (
    <div style={{ padding: '14px 20px 16px' }}>
      {/* waveform */}
      <div ref={ref} onClick={seekAt}
        onMouseMove={(e) => { const r = ref.current?.getBoundingClientRect(); if (!r) return; setHover({ t: Math.max(0, Math.min(1, (e.clientX - r.left) / r.width)) * dur, x: e.clientX - r.left }); }}
        ...
        {waveform.map((v, i) => {
          ...
          return <div key={i} style={{ flex: 1, height: `${v * 100}%`, background: played ? 'var(--cyan)' : 'rgba(255,255,255,0.13)', borderRadius: 1 }} />;
        })}
      ...
      <div style={{ position: 'relative', height: 20, marginTop: 3 }}>
        {notes.map((n) => {
          const cp = n.t / dur, active = activeNote === n.id, c = n.pinned ? 'var(--cyan)' : 'var(--violet)';
          return (
            <button type="button" key={n.id} onClick={() => onNoteClick(n)} title={n.text}
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginTop: 12 }}>
        <button type="button" onClick={onTogglePlay} style={{ width: 44, height: 44, borderRadius: '50%', background: 'var(--cyan)', color: '#06151a', display: 'grid', placeItems: 'center', flexShrink: 0, fontSize: 14, boxShadow: playing ? '0 0 22px rgba(0,229,176,0.55), 0 0 0 1px rgba(0,229,176,0.6)' : '0 0 12px rgba(0,229,176,0.3)' }}>{playing ? '⏸' : '▶'}</button>
        <button type="button" className="btn ghost sm" style={{ width: 34, height: 34, padding: 0, justifyContent: 'center' }} onClick={() => onSeek(Math.max(0, position - 10))}>⏮</button>
        <button type="button" className="btn ghost sm" style={{ width: 34, height: 34, padding: 0, justifyContent: 'center' }} onClick={() => onSeek(Math.min(dur, position + 10))}>⏭</button>
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 14 }}>
          ...
          <button type="button" className="btn sm">+ Note @ {fmtTime(position)}</button>
        </div>
      </div>
    </div>
  );
