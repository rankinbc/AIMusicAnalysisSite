...
- `aimusic_shared/models.py` (~1370 lines) — SQLAlchemy 2.0 declarative mirror of the EF schema. `Base`, string job-status constants (`JOB_STATUS_PENDING/PROCESSING/COMPLETE/FAILED/AWAITING_STEM_MAPPING`, `JOB_STATUS_ALL` — the former `JobStatus` enum is gone), and mapped classes covering:
  - Core: `User`, `RefreshToken`, `Song`, `SongVersion`, `AnalysisJob` (nullable `user_id`/`device_id` for the anon funnel, `file_path` for song-less anon jobs, `tier` stamped by the BFF at dispatch, `error_code`, `retry_of_job_id`), `Analysis` (`final_json`, `phase_durations`, `routing_plan`, `degradation_notice`, result-image paths).
  ...
  - Listen/rack + reference tables: `SessionNote`, `RackPreset`, `RackDraft`, `VizPreset`, `ReferenceTrack`, `ReferenceSet(+Member)`, `CompareCache`, `Notification` (email send-ledger only — no read API; see `docs/data-models.md`). `ShareSetting`, `Invite`, `ListeningSession`, `ControlGrant`, `TrackComment`, `ReviewerSuggestion`, `TrackBookmark`, and `FollowRelation` were all removed by the `RemoveSocial` migration (solo fork — see `PRPs/solo-fork-strip-social.md`); there is no live-room, sharing, invite, reviewer-comment/suggestion, bookmark, or follow feature any more.
