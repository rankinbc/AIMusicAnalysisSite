...
- **Legacy Alembic is frozen** — `migrations/` at repo root serves only the legacy FastAPI v1 tables (`alembic -c migrations/alembic.ini upgrade head`); no new Alembic revisions. The worker never migrates anything.
